...
class EEGModel(nn.Module):
    def __init__(self, num_classes=5, lstm_hidden_size=100):
        ...
        # --- Convolutional Block ---
        # These layers process each time window (frame) individually.
        # We assume each frame is a 32x3 matrix (32 channels, 3 frequency band magnitudes).
        # We first add a channel dimension so each frame becomes (1, 32, 3).
        ...
        # After two poolings on the channel dimension: 32 -> 16 -> 8.
        # The width (3 frequency bands) remains unchanged.
        # So, flattened size per frame = 64 channels * 8 * 3 = 1536.
        self.flattened_size = 64 * 8 * 3
        
        # --- LSTM Layer ---
        # This layer processes the sequence of frames (each represented by a 1536-dimensional feature vector)
        ...
        # These layers take the final output of the LSTM and further process it to produce the final classification.
...
if __name__ == "__main__":
    # Simulate a batch of data: batch_size=8, T=10 time windows, 32 channels, 3 frequency magnitudes
    sample_input = torch.randn(8, 10, 32, 3)
    model = EEGModel(num_classes=5, lstm_hidden_size=100)
    output = model(sample_input)
    print("Output shape:", output.shape)  # Expected: (8, 5)
